scoreboard objectives add HUD trigger {text:"Render HUD",color:"gold",click_event:{action:"run_command",command:"/trigger HUD"}}
scoreboard objectives add mk.hud.slept minecraft.custom:minecraft.time_since_rest
scoreboard objectives add mk.hud.ticks minecraft.custom:minecraft.play_time
scoreboard players set #10 mk.math 10
scoreboard players set #2 mk.math 2
scoreboard players set #1200 mk.math 1200
scoreboard players set #24 mk.math 24
scoreboard players set #8 mk.math 8
scoreboard players set #1000 mk.math 1000
scoreboard players set #3 mk.math 3
scoreboard players set #50 mk.math 50
scoreboard players set #12000 mk.math 12000
scoreboard players set #12 mk.math 12